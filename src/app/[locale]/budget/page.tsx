...
import { useBudgetTranslations, useCommonTranslations } from '@/hooks/useTranslations';
...
export default function BudgetPage() {
  ...
  if (status === "loading") {
    return <main className="p-4"><p>{common('loading')}</p></main>;
  }

  if (!session) {
    // 或者重定向到登录页
    return <main className="p-4"><p>{t('description')}</p></main>;
  }
  ...
  return (
    <main className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white shadow-md rounded-lg p-6 mb-6">
        <h2 className="text-xl font-semibold text-gray-700 mb-2">{t('currentBudget')}</h2>
        {currentBudget !== null && currentBudget !== undefined ? (
          <p className="text-2xl font-bold text-indigo-600">
            {currentBudget.toLocaleString()} {session.user?.currency}
          </p>
        ) : (
          <p className="text-gray-500">{t('noBudgetSet')}</p>
        )}
        <button
        ...
        >
          {currentBudget !== null && currentBudget !== undefined ? t('editBudget') : t('setBudget')}
        </button>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div className="flex justify-between items-center p-4 border-b">
              <h2 className="text-lg font-semibold text-gray-700">
                {currentBudget !== null && currentBudget !== undefined ? t('budgetModal.editTitle') : t('budgetModal.setTitle')}
              </h2>
              <button
