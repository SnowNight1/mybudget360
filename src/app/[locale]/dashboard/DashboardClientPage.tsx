...
export default function DashboardClientPage({
    ...
    const handleTransactionSuccess = (processedExpense: ExpenseWithCategory) => {
        ...
        if (editingExpenseId) { // 如果是编辑模式
            alert(t('messages.expenseUpdated', { description: processedExpense.note || processedExpense.amount }));
            ...
            alert(t('messages.expenseAdded', { description: processedExpense.note || processedExpense.amount }));
            // 新增的记录放在最前面
    ...
    const handleDelete = async (expenseId: number, expenseDescription: string) => {
      if (!window.confirm(t('messages.deleteConfirm', { description: expenseDescription }))) {
        return;
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          const errorData = await response.json().catch(() => ({message: t('messages.deleteFailed')}));
          console.error('删除消费记录失败 API response:', errorData);
          alert(t('messages.deleteServerError', { message: errorData.message || '服务器返回错误，但未提供详细信息。' }));
        }
      } catch(error) {
  ...
  // 检查并生成订阅账单
        ...
        try {
          ...
          if (response.ok) {
            ...
            if (result.generatedCount > 0) {
              console.log(t('messages.subscriptionBillsGenerated', { count: result.generatedCount }));
              // 刷新支出列表以包含新生成的账单
              await refreshExpenses();
            }
          }
        } catch (error) {
          console.error(t('messages.checkBillsFailed'), error);
        }
      };
    ...
    const refreshExpenses = async () => {
      try {
        ...
        console.error(t('messages.refreshFailed'), error);
      }
    };
      ...
      <main className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600">{t('defaultCurrency', { currency: userCurrency })}</p>
          </div>
          <button
        ...
        <div className="bg-white p-6 rounded-lg shadow mb-6">
          <h2 className="text-lg font-medium text-gray-700">{t('budgetOverview')}</h2>
          {userMonthlyBudget !== null && userMonthlyBudget !== undefined ? (
            <div className='space-y-1'>
            <p className="text-gray-700">
            {t('monthlyBudget')}: <span className="font-semibold">{userMonthlyBudget.toLocaleString()} {userCurrency}</span>
            </p>
            <p className='text-gray-700'>
              {t('monthlySpent')}：
        ...
        {expenses.length > 0 || monthOptionsForSelect.length>0 ? (
          <div className="bg-white p-6 rounded-lg shadow space-y-8">
            <h2 className="text-xl font-semibold text-gray-700 mb-4">{t('spendingAnalysis')}</h2>
            {/*全局图表控件：月份选择 */}
            ...
            {/* <div className="border-t pt-6"> */}
              <div className=" flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3">
                <h3 className="text-lg font-medium text-gray-700">{t('monthlySpendingTrend')}</h3>
                <select
            ...
            <div className="border-t pt-6">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3">
                    <h3 className="text-lg font-medium text-gray-700">{t('categorySpendingDistribution')}
        ...
        <div className="bg-white p-4 md:p-6 rounded-lg shadow">
          <h2 className="text-lg font-medium text-gray-700 mb-4">{t('recentTransactions')}</h2>
          {expenses.length === 0 ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    >
                      {t('tableHeaders.amount')} ({userCurrency})
                    </th>
                      <th
                      scope="col"
                      className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider"
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-white rounded-lg shadow-xl max-w-lg w-full my-8">
              <div className="flex justify-between items-center p-4 border-b">
                <h2 className="text-lg font-semibold">{t('modal.addTransaction')}</h2>
                <button onClick={handleCloseModal} className="text-gray-500 hover:text-gray-700 text-2xl leading-none p-1">{t('modal.close')}</button>
              </div>
              <TransactionForm
                categories={categories}
