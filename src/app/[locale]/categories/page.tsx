...
export default function CategoriesPage() {
  ...
  const handleDeleteCategory = async (categoryId: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || `${t('deleteFailed')}，状态码: ${response.status}`);
      }
      // 删除成功
  ...
  if (loading && categories.length === 0) return <p className="text-center py-10">{t('loading')}</p>; // 只有在首次加载时显示
  if (error) return <p className="text-center py-10 text-red-500">{t('loadingError')}: {error}</p>;
    ...
    <main className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      */}
      <div>
              ...
              >
                <div className="flex items-center">
                  ...
                  {category.parentId && ( // 显示父级 ID (如果存在)
                    <span className="text-xs text-gray-500 ml-2">({t('parentId')}: {category.parentId})</span>
                  )}
                </div>
        ...
        {loading && categories.length > 0 && <p className="text-center py-4">{t('updating')}</p>} {/* 列表已有时，显示更新中 */}
      </div>
